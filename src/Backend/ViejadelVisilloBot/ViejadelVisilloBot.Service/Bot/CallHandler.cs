...
namespace ViejadelVisilloBot.Services.Bot
{
    /// <summary>
    /// Call Handler Logic.
    /// </summary>
    public class CallHandler : HeartbeatHandler
    {
        ...
        public CallHandler(ICall statefulCall, BotConfiguration botConfiguration, ILogger<LogBase> ailogger)
            : base(TimeSpan.FromMinutes(10), statefulCall?.GraphLogger)
        {
            ...
            {
                ailogger.LogInformation($"SMARTBOT | ERROR PSI PIPELINE ERROR: {ex.Exception.Message}");
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            ailogger.LogInformation($"SMARTBOT | CallHandler: Dispose participants");
        ...
        {
            this.GraphLogger.Info($"SMARTBOT | Call status updated to {e.NewResource.State} - {e.NewResource.ResultInfo?.Message}");
            // Event - Recording update e.g established/updated/start/ended

            ailogger.LogInformation($"SMARTBOT | CallOnUpdate: Call status updated to {e.NewResource.State} - {e.NewResource.ResultInfo?.Message}");

            if (e.OldResource.State != e.NewResource.State && e.NewResource.State == CallState.Established)
            {
                Task.Run(async () => { await this.Call.UpdateRecordingStatusAsync(RecordingStatus.Recording); });
            }
        }
        ...
        private string CreateParticipantUpdateJson(string participantId, string participantDisplayName = "")
        {
            ...
            {
                ailogger.LogInformation($"SMARTBOT | Create participant {participantId}");
                return "{" + String.Format($"\"Id\": \"{participantId}\"") + "}";
            }
            else
            {
                ailogger.LogInformation($"SMARTBOT | Create participant {participantId} | {participantDisplayName}");
        ...
        private string UpdateParticipant(List<IParticipant> participants, IParticipant participant, bool added, string participantDisplayName = "")
        {
            if (added)
            {
                ailogger.LogInformation($"SMARTBOT | Participant Added {participant.Id}");
            ...
            {
                ailogger.LogInformation($"SMARTBOT | Participant Removed {participant.Id}");
        ...
        private void UpdateParticipants(ICollection<IParticipant> eventArgs, bool added = true)
        {
            foreach (var participant in eventArgs)
            {
                ...
                {
                    ailogger.LogInformation($"SMARTBOT | Update participant: {participantDetails.DisplayName}");
        ...
        {
            ailogger.LogInformation($"SMARTBOT | [{this.Call.Id}:OnDominantSpeakerChanged(DominantSpeaker={e.CurrentDominantSpeaker})]");
        ...
        private void SubscribeToParticipantVideo(IParticipant participant, bool forceSubscribe = true)
        {
            ...
            if (participantSendCapableVideoStream != null)
            {
                ...
                lock (this.subscriptionLock)
                {
                    if (this.currentVideoSubscriptions.Count < this.Call.GetLocalMediaSession().VideoSockets.Count)
                    {
                        ...
                        ailogger.LogInformation($"SMARTBOT | [{this.Call.Id}:SubscribeToParticipant(socket {socketId} " +
                            $"available, the number of remaining sockets is {this.availableSocketIds.Count}, subscribing to the participant {participant.Id})");
                ...
                if (subscribeToVideo && socketId != uint.MaxValue)
                {
                    ...
                    ailogger.LogInformation($"SMARTBOT | [{this.Call.Id}:SubscribeToParticipant(subscribing to the participant {participant.Id} " +
                        $"on socket {socketId})");
            ...
            {
                // new sharer
                ailogger.LogInformation($"SMARTBOT | [{this.Call.Id}:SubscribeToParticipant(subscribing to the VBSS sharer {participant.Id})");
